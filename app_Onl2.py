...
def score_inflammation(age, hscrp, nlr=None):
    ...
    drivers = []
    if not is_nan(hscrp):
        drivers.append(f"hsCRP {hscrp:.2f} mg/L")
...
def score_lipids(ldl_mgdl, hdl_mgdl, tg_mgdl):
    ...
    drivers = []
    if not is_nan(ldl_mgdl):
        drivers.append(f"LDL {ldl_mgdl:.0f} mg/dL")
    if not is_nan(tg_hdl):
        drivers.append(f"TG/HDL {tg_hdl:.2f}")
...
def score_glucose(hba1c, glucose_mgdl, insulin_uIU_ml=None):
    ...
    drivers = []
    if not is_nan(hba1c):
        drivers.append(f"HbA1c {hba1c:.2f}%")
    if not is_nan(glucose_mgdl):
        drivers.append(f"Glucosa {glucose_mgdl:.0f} mg/dL")
...
def score_hematology(sex_code, hb, mcv, rdw, ferritin, vitb12=None, folate=None):
    ...
    if vitb12 is not None and (not is_nan(vitb12)):
        b12_r = clamp01((350.0 - vitb12) / 200.0)

    fol_r = 0.0
    if folate is not None and (not is_nan(folate)):
        fol_r = clamp01((6.0 - folate) / 3.0)

    score = 100.0 * (0.40 * hb_r + 0.28 * rdw_r + 0.14 * fer_low_r + 0.10 * mcv_r + 0.04 * b12_r + 0.04 * fol_r)

    drivers = []
    if not is_nan(hb):
        drivers.append(f"Hb {hb:.2f} g/dL (umbral {hb_thr:.1f})")
    if not is_nan(rdw):
        drivers.append(f"RDW {rdw:.2f}%")
    if not is_nan(mcv):
        drivers.append(f"MCV {mcv:.1f} fL")
    if not is_nan(ferritin):
        drivers.append(f"Ferritina {ferritin:.0f} ng/mL")
    if vitb12 is not None and not is_nan(vitb12):
        drivers.append(f"B12 {vitb12:.0f} pg/mL")
    if folate is not None and not is_nan(folate):
        drivers.append(f"Folato {folate:.1f} ng/mL")
...
def score_liver(ast, alt, ggt=None, alp=None, bili=None):
    ...
    drivers = []
    if not is_nan(alt):
        drivers.append(f"ALT {alt:.0f} U/L")
    if not is_nan(ast):
        drivers.append(f"AST {ast:.0f} U/L")
    if ggt is not None and not is_nan(ggt):
        drivers.append(f"GGT {ggt:.0f} U/L")
    if alp is not None and not is_nan(alp):
        drivers.append(f"ALP {alp:.0f} U/L")
    if bili is not None and not is_nan(bili):
        drivers.append(f"Bilirrubina {bili:.2f} mg/dL")
...
USED_KEYS_V1 = {
    "inflammation": ["hscrp_mg_l", "neut_abs_x10_3_mm3", "lymph_abs_x10_3_mm3"],
    "lipids": ["ldl_mg_dl", "hdl_mg_dl", "triglycerides_mg_dl"],
    "glucose": ["glucose_mg_dl", "hba1c_pct", "insulin_uIU_ml"],
    "hematology": ["hb_g_dl", "mcv_fl", "rdw_pct", "ferritin_ng_ml", "vitb12_pg_ml", "folate_ng_ml"],
    "liver": ["ast_u_l", "alt_u_l", "ggt_u_l", "alp_u_l", "bilirubin_total_mg_dl"],
    "kidney": ["creatinine_mg_dl", "egfr_ml_min_1_73m2", "uric_acid_mg_dl"],
}
...
def get_items_by_level(schema, level):
    return [i for i in schema if i.get("level") == level]


def get_items_by_domain(schema, domain):
    return [i for i in schema if i.get("domain") == domain]


def completeness_for_items(items, values):
    total = len(items)
    if total == 0:
        return 0.0, 0, 0
    filled = sum(1 for item in items if has_value(values.get(item["key"], np.nan)))
    return filled / total, filled, total


def compute_confidence(schema, values):
    core_items = get_items_by_level(schema, "core")
    plus_items = get_items_by_level(schema, "plus")

    core_cov, core_n, core_total = completeness_for_items(core_items, values)
    plus_cov, plus_n, plus_total = completeness_for_items(plus_items, values)

    score = 0.75 * core_cov + 0.25 * plus_cov
...
def coverage_for_used_keys(values):
    out = {}
    for domain, keys in USED_KEYS_V1.items():
        total = len(keys)
        filled = sum(1 for k in keys if has_value(values.get(k, np.nan)))
...
def rank_domains(scores):
    paired = list(zip(SCORE_DOMAIN_ORDER, scores))
    paired_sorted = sorted(paired, key=lambda x: x[1], reverse=True)

    ranked = []
    for (domain_key, domain_name), score in paired_sorted:
        ranked.append({
            "key": domain_key,
            "name": domain_name,
            "score": float(score),
            "bucket": bucket(float(score))
        })
    return ranked


def global_status(scores):
    worst = max(scores)
    if worst < 30:
        return "Bajo"
    if worst < 60:
...
def decouple_label(decouple):
    if decouple < 10:
        return "Bajo (perfil bastante alineado)"
    if decouple < 20:
        return "Medio (hay un cuello de botella principal)"
    return "Alto (perfil mixto o ejes desalineados)"


def interpret_profile(scores, derived, ranked_domains):
    infl, lip, glu, hem, liv, ren = scores
    tg_hdl = derived.get("tg_hdl_ratio", np.nan)
    homa = derived.get("homa_ir", np.nan)

    primary = ranked_domains[0]["name"]
    secondary = ranked_domains[1]["name"]

    if hem >= 70:
        return (
            "El eje hematológico domina el perfil actual. "
            "Tiene sentido priorizar la corrección de hierro/B12/folato o causas inflamatorias antes de intervenciones dietéticas agresivas."
        )

    if glu >= 60 and (((not is_nan(homa)) and homa >= 2.5) or ((not is_nan(tg_hdl)) and tg_hdl >= 2.5)):
        return (
            "El patrón dominante es glucémico-metabólico, compatible con una palanca principal en carga glucémica e insulinorresistencia. "
            f"El eje secundario es {secondary.lower()}, lo que sugiere una interacción metabólica más que un problema aislado."
        )

    if infl >= 60 and glu < 55:
        return (
            "El perfil parece predominantemente inflamatorio, con glucosa relativamente estable. "
            "La primera palanca razonable sería un enfoque antiinflamatorio y de reducción de carga fisiológica."
        )

    if lip >= 60:
        return (
            "El eje lipídico domina el perfil actual. "
            "La combinación de calidad de grasa, fibra soluble y reducción de carga glucémica puede ser la intervención más eficiente."
        )

    if max(scores) < 30:
        return (
            "El perfil global está bastante alineado y no destaca un eje claramente problemático. "
            "Aquí la prioridad parece más de optimización que de corrección."
        )

    return (
        f"El perfil es mixto, con prioridad principal en {primary.lower()} y secundaria en {secondary.lower()}. "
        "Conviene intervenir sobre el eje dominante sin perder de vista el secundario."
    )


def get_used_unused_markers(schema, values):
    ...
    for domain, keys in USED_KEYS_V1.items():
        for k in keys:
            if k in schema_keys:
                ...
                used.append({
                    "key": k,
                    "label": schema_keys[k]["label"],
                    "domain": DOMAIN_LABELS.get(schema_keys[k]["domain"], schema_keys[k]["domain"]),
                    "has_value": has_value(values.get(k, np.nan))
                })

    unused = []
    for item in schema:
        if item["key"] not in used_key_set:
            unused.append({
                "key": item["key"],
                "label": item["label"],
                "domain": DOMAIN_LABELS.get(item["domain"], item["domain"]),
                "has_value": has_value(values.get(item["key"], np.nan))
            })

    return used, unused


def build_missing_focus_message(domain_cov):
    important_domains = ["glucose", "lipids", "inflammation", "hematology", "liver", "kidney"]

    weak = []
    for d in important_domains:
        if d in domain_cov and domain_cov[d]["coverage"] < 0.40:
            weak.append(domain_cov[d]["name"])

    if not weak:
        return "Cobertura razonable para la v1."
    if len(weak) == 1:
        return f"Faltan datos sobre todo en el eje de {weak[0].lower()}."
    return "Faltan datos sobre todo en: " + ", ".join([w.lower() for w in weak[:-1]]) + f" y {weak[-1].lower()}."


def build_priority_actions(actions, ranked_domains):
    primary = ranked_domains[0]["name"]
    secondary = ranked_domains[1]["name"]

    out = [
        f"Prioridad principal: **{primary}**.",
        f"Prioridad secundaria: **{secondary}**.",
    ]
    out.extend(actions[:4])
    return out
...
# Output intelligence
# -----------------------------
confidence = compute_confidence(SCHEMA_V1, values)
domain_cov = coverage_for_used_keys(values)
ranked_domains = rank_domains(scores)
profile_text = interpret_profile(scores, derived, ranked_domains)
overall_status = global_status(scores)
decouple_text = decouple_label(decouple)
used_markers, unused_markers = get_used_unused_markers(SCHEMA_V1, values)
missing_focus_msg = build_missing_focus_message(domain_cov)
priority_actions = build_priority_actions(actions, ranked_domains)

primary_domain = ranked_domains[0]
secondary_domain = ranked_domains[1]
...
with left:
    st.markdown(f"### Prioridad principal: **{primary_domain['name']}**")
    st.write(profile_text)
    st.markdown(f"**Patrón dietético sugerido:** {diet}")
    st.markdown(f"**Estado global:** {overall_status}")
    st.markdown(f"**Confianza del resultado:** {confidence['label']}")
    st.caption(
        f"Core completos: {confidence['core_n']}/{confidence['core_total']} · "
        f"Plus completos: {confidence['plus_n']}/{confidence['plus_total']}"
    )
    st.caption(missing_focus_msg)

with right:
    st.info(
        f"**Secundario:** {secondary_domain['name']}\n\n"
        f"**Desacople:** {decouple:.1f}\n\n"
        f"{decouple_text}"
    )
...
st.markdown("---")
st.subheader("Scorecard (0 mejor → 100 peor)")
...
st.caption(
    f"Dominio principal: {primary_domain['name']} ({primary_domain['score']:.1f}) · "
    f"Dominio secundario: {secondary_domain['name']} ({secondary_domain['score']:.1f})"
)
...
st.markdown("### Racional de la dieta sugerida")
...
cov_map = {
    "inflammation": cov_cols[0],
    "lipids": cov_cols[1],
    "glucose": cov_cols[2],
    "hematology": cov_cols[3],
    "liver": cov_cols[4],
    "kidney": cov_cols[5],
}

for domain_key, col in cov_map.items():
    if domain_key in domain_cov:
        d = domain_cov[domain_key]
        with col:
            st.metric(d["name"], f'{d["filled"]}/{d["total"]}', d["label"])
...
with st.expander("Marcadores usados en la interpretación v1", expanded=False):
    for item in used_markers:
        mark = "✅" if item["has_value"] else "—"
        st.write(f"- {mark} {item['label']} ({item['domain']})")

with st.expander("Marcadores disponibles pero no interpretados aún en v1", expanded=False):
    shown = 0
    for item in unused_markers:
        if item["has_value"]:
            st.write(f"- {item['label']} ({item['domain']})")
            shown += 1
    if shown == 0:
        st.write("- No hay marcadores adicionales informados fuera del motor actual.")
